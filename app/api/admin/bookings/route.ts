  ...
  try {
    ...
    if (status) {
      ...
      if (status === "cancelled") {
        ...
        updates.cancellation_reason = notes || "Cancelled by admin";
      } else {
    ...
    if (hasGoogleConfig && booking) {
      try {
        ...
        console.error(
          "[Admin Bookings] Failed to sync Google Calendar:",
          calendarError,
        );
      ...
      if (hasResendConfig && userData?.email) {
        try {
          ...
          if (emailResult.success) {
            ...
            console.error(
              "[Admin Bookings] Email confirmation failed:",
              emailResult.error,
            );
          }
        } catch (emailError) {
          console.error(
            "[Admin Bookings] Failed to send email confirmation:",
            emailError,
          );
        }
      }
    ...
    try {
      ...
      await logOriginalBooking({
        ...
        notes:
          existingBooking.notes || "Original booking before admin modification",
      ...
      console.error(
        "[Admin Bookings] Failed to log to Google Sheet:",
        sheetError,
      );
    }
...
export async function DELETE(request: NextRequest) {
  ...
  if (hasGoogleConfig && existingBooking.google_event_id) {
    try {
      ...
      console.error(
        "[Admin Bookings] Failed to delete Google Calendar event:",
        calendarError,
      );
      // Don't fail the request, just log the error
    }
  ...
  try {
    ...
    console.error(
      "[Admin Bookings] Failed to log deletion to Google Sheet:",
      sheetError,
    );
  }
